const audioFiles = [
	"AIN'T NO WAY.wav",
	"bazinga (1).wav",
	"bazinga (2).wav",
	"bazinga (3).wav",
	"bazinga (4).wav",
	"bazinga (5).wav",
	"bazinga (6).wav",
	"bozo (1).wav",
	"bozo (2).wav",
	"busting a nut.wav",
	"Clench my ass.mp3",
	"cocka.wav",
	"Come on mario.mp3",
	"crushed by king whomp.wav",
	"edging.wav",
	"eheh.mp3",
	"fuck a fan canceled (1).wav",
	"fuck a fan canceled (2).wav",
	"fuck a fan is back on.wav",
	"goated (1).wav",
	"goated (2).wav",
	"GULP.wav",
	"I am going to cum.wav",
	"I fuck with you heavy (1).wav",
	"I fuck with you heavy (2).wav",
	"I fuck with you heavy (3).wav",
	"I'm-a going to cum.mp3",
	"Jerma confirms Squeex is awesome.wav",
	"JOI.wav",
	"koopas (1).wav",
	"koopas (2).wav",
	"koopas (3).wav",
	"kramer jerry.wav",
	"late_night_squeexin.mp3",
	"let's go brandon.wav",
	"Make your bed in the morning (1).mp3",
	"Make your bed in the morning (2).mp3",
	"marker bit eraser.wav",
	"marker bit.wav",
	"meow.wav",
	"might be wondering (1).wav",
	"might be wondering (2).wav",
	"might be wondering (3).wav",
	"minecraft villager.wav",
	"More at 7 (1).mp3",
	"More at 7 (2).mp3",
	"More at 7 (3).mp3",
	"More at 7 (4).mp3",
	"More at 7 (5).mp3",
	"no cap.wav",
	"obama sasha malia (1).wav",
	"obama sasha malia (2).wav",
	"obama stimulate your economy.wav",
	"obama y'know.wav",
	"ohoh.mp3",
	"pamplemousse.wav",
	"pirate ain't no way.wav",
	"pirate oh the huge manatee.wav",
	"pirate oh the misery.wav",
	"quieres.wav",
	"rock hard for whomps.wav",
	"SAJ.mp3",
	"seinfeld kramer (1).wav",
	"seinfeld kramer (2).wav",
	"seinfeld speed walking.wav",
	"seinfeld what's up with that.wav",
	"sheesh (1).wav",
	"sheesh (2).wav",
	"show us your pants.wav",
	"Spongebob O sounds.mp3",
	"squeex discovers the soundboard.wav",
	"Squeex play minecwaf.wav",
	"Stardew valley harvesting berries.wav",
	"testicles.wav",
	"Thanks for the head.mp3",
	"that just happened (1).wav",
	"that just happened (2).wav",
	"This goes out to all the believers.wav",
	"This is BBC news.mp3",
	"This is my fucking nightmare.mp3",
	"Toad awawawawa.mp3",
	"toomyah.mp3",
	"walkin (1).wav",
	"walkin (2).wav",
	"walkin (3).wav",
	"Wanna go for a ride jump on my hog.mp3",
	"watch me cum.wav",
	"well gary (1).wav",
	"well gary (2).wav",
	"well gary (3).wav",
	"well gary (4).wav",
	"witch A.wav",
	"witch CINNA.wav",
	"yoshi (1).wav",
	"yoshi (2).wav",
	"yoshi (3).wav",
	"zany life.wav",
];

const CONTAINER = document.getElementById('list-container');
const TEMPLATE = document.getElementById('template');
const AUDIO_DIR = 'audio';

audioFiles.forEach(filename => {
	const node = TEMPLATE.cloneNode(true);
	node.removeAttribute('hidden');
	node.removeAttribute('id');

	const title = node.querySelector('div');
	const source = node.querySelector('source');

	title.innerText = filename.split('.')[0];
	const ext = filename.split('.').pop().toLowerCase();
	let type;
	if (ext === 'mp3') {
		type = 'audio/mpeg';
	} else if (ext === 'wav') {
		type = 'audio/wav';
	}
	source.setAttribute('src', AUDIO_DIR + '/' + filename);
	source.setAttribute('type', type);

	CONTAINER.append(node);
});
